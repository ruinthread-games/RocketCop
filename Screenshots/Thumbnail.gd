...
func generate_level():
	...
	for i in range(blocks_rows):
		for j in range(blocks_cols):
			...
			var grid_offset = Vector3(-1*GRID_WIDTH*(i-1),0,-1*GRID_WIDTH*(j+2)) + random_offset
...
func _ready():
	...
	$PlayerCharacter.set_ui_visible(false)


# Called every frame. 'delta' is the elapsed time since the previous frame.
func _process(delta):
	$PlayerCharacter.aim_down_sights_progress = 1.0
	$PlayerCharacter.get_node("MeshPivot/Armature/Skeleton/JetpackBoneAttachment/RightThrusterParticles").get_process_material().damping = 0
